haste () {
	...
	output=$(curl -# -f -XPOST "https://hastebytrhacknon.trhacknon.repl.co/documents" -d"$contents")
	if (( $? == 0 )) && [[ $output =~ \"key\" ]]
		then
		returnfile=$(sed 's/^.*"key":"/https:\/\/hastebytrhacknon.trhacknon.repl.co\//;s/".*$//' <<< "$output")
		if [[ -n $returnfile ]]
			then
			echo "$returnfile"
			return 0
